...
if (!class_exists('CSF_Options')) {
    class CSF_Options extends CSF_Abstract
    {
        ...
        public $args = array(
            ...
            // framework title
            'framework_title' => 'BearGallery OptionsFramework',
            'framework_class' => '',
...
//            }
        ...
        public function set_options($ajax = false)
        {
            ...
            $response = ($ajax && !empty($_POST['data'])) ? json_decode((trim($_POST['data'])), true) : $_POST;

            // Set variables.
            $data = array();
            $noncekey = 'csf_options_nonce' . $this->unique;
            $nonce = (!empty($response[$noncekey])) ? $response[$noncekey] : '';
            $options = (!empty($response[$this->unique])) ? $response[$this->unique] : get_option($this->unique);
...
//                var_dump(get_option($this->unique));
...
//      do_action( 'csf_options_before' );
            ...
            // add plugin
            echo '<input type="hidden" class="" name="plugin" value="'.CSF::$plugin_name.'">';
