        ...
        public static function get_enqueue_style($header = null)
        {
            ...
            if ($loads_statics) { // if is plugin
              ...
              $style = '<link rel="stylesheet" href="//lf9-cdn-tos.bytecdntp.com/cdn/expire-1-M/font-awesome/5.15.4/css/all.min.css?ver=5.15.4">' .
                    ...
                    '<link href="https://lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/fomantic-ui/2.8.8/semantic.min.css" type="text/css" rel="stylesheet" />' .
...
margin-left:20px;
                    ...
                    '<script>_wpUtilSettings={ajax:{url: "'. Common::url('/bgoptions/ajax', Helper::options()->index).'"}};window.ajaxurl=_wpUtilSettings.ajax.url;</script>';
        ...
        public static function get_enqueue_script($old = null)
        {
            ...
            if ($loads_statics) { // if is plugin
                ...
                $script = '<script src="' . self::include_plugin_url('assets/js/lodash.min.js') . '?v=1"></script>' .
                      '<script src="' . self::include_plugin_url2('assets/js/plugins.min.js') . '?v=1"></script>' .
                    '<script src="' . self::include_plugin_url2('assets/js/main.min.js') . '?v=2"></script>' .
                    '<script src="' . self::include_plugin_url2('assets/js/backend.js') . '?v=2"></script>' .
                    '<script src="' . self::include_plugin_url2('assets/js/iris.min.js') . '?v=1"></script>' .
                    '<script src="//lf6-cdn-tos.bytecdntp.com/cdn/expire-1-M/jquery-tokeninput/1.7.0/jquery.tokeninput.min.js" type="application/javascript"></script>'.
                    '<script src="' . self::include_plugin_url2('assets/js/color-picker.min.js') . '?v=1"></script>' .
                    '<script src="' . self::include_plugin_url2('assets/js/jquery/ui/core.min.js') . '?v=1"></script>' .
                    '<script src="' . self::include_plugin_url2('assets/js/jquery/ui/draggable.min.js') . '?v=1"></script>' .
                    '<script src="' . self::include_plugin_url2('assets/js/jquery/ui/mouse.min.js') . '?v=1"></script>' .
                    '<script src="' . self::include_plugin_url2('assets/js/jquery/ui/menu.min.js') . '?v=1"></script>' .
                    '<script src="//lf9-cdn-tos.bytecdntp.com/cdn/expire-1-M/toastr.js/2.1.4/toastr.min.js" type="application/javascript"></script>'.
                    '<script src="//lf3-cdn-tos.bytecdntp.com/cdn/expire-1-M/fomantic-ui/2.8.8/semantic.min.js" type="application/javascript"></script>' .
                    '<script>$(function(){$(\'.menu .item\').tab();})</script>'.
...
//            self::include_plugin_file('views/welcome.php');
            ...
            $form_action = Helper::security()->getIndex('/bgoptions/save?plugin=' . $pluginName);
            ...
            if (class_exists('CSF_Options') && !empty(self::$args['admin_options'])) {
                foreach (self::$args['admin_options'] as $key => $value) {
                    ...
                    if (!empty(self::$args['sections'][$key]) && !isset(self::$inited[$key])) {
                        ...
                        update_bg_key_params($key, $op_obj);
...
//            do_action('csf_loaded');
        ...
        {
            $val = new \Typecho\Widget\Helper\Form\Element\Hidden('bgOptionsHide',
                NULL,
                "bgOptionsHide",
                'bg选项框架隐藏val',
                '');
...
//      $directory_uri  = set_url_scheme( $directory_uri, $protocol_uri );
        ...
        public static function include_plugin_url2($file)
        {
            ...
            return '/usr/plugins/BgCore/'.$file;
        }
        // Include files
        public static function includes()
        {
            // include router
            self::include_plugin_file('classes/bgRouter.class.php');
