...
const FRAMEWORK_PLUGIN_NAME = 'plugin:BgCore_';
const FRAMEWORK_KEY_PARMAS_NAME = 'plugin:bgOF_key_params';
...
if (!function_exists('update_bg_key_params')) {
    function update_bg_key_params($option, $value, $autoload = null)
    {
        $db = Db::get();
...
if (!function_exists('get_bg_key_params')) {

// get option for framework
    function get_bg_key_params($option, $default = false)
    {
        $db = Db::get();
        $pluginName = FRAMEWORK_KEY_PARMAS_NAME;
        $select = $db->select()->from('table.options')
