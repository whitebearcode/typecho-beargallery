...
class General
{
    ...
    //获取文章图片
    public static function getArticleImage($obj,$place){
        $content = $obj->content;
        $pattern = '/<\s*img[\s\S]+?(?:src=[\'"]([\S\s]*?)[\'"]\s*|alt=[\'"]([\S\s]*?)[\'"]\s*|[a-z]+=[\'"][\S\s]*?[\'"]\s*)+[\s\S]*?>/i';

 $replacement = '
  <img data-fancybox="post-'.$obj->cid.'" data-caption="$2" src="$1" alt="$2" style="border-radius:3px;cursor: zoom-in;" loading="lazy">'; 
    ...
    public static function getCategoryDescriptionData($text)
{
   ...
   return $postnum['mid']; 
    ...
    switch($type){
       case 'search':
          return Helper::options()->siteUrl.'index.php/bg-search';
         break; 
    }
